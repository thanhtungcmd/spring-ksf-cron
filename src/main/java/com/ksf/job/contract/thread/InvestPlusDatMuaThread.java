...
public class InvestPlusDatMuaThread extends Thread {
    ...
    public InvestPlusDatMuaThread(Queue<OrderList.OrderListData.OrderListDataList.OrderListDataListItem> queue, String token) {
    ...
    public void run() {
        try {
            ...
            while ((item = this.queue.poll()) != null) {
                this.hopDongDatMuaMeta(item, this.token);
            }
        } catch (Exception e) {
            logger.error(e);
            e.printStackTrace();
